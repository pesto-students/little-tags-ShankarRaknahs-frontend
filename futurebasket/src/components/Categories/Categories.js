...
function Categories() {
    ...
    const handleCategory = (selectedCategory) =>  setSelectedCategory(selectedCategory); 
            ...
            <div className = "categories">
                {categories.map(category => category === 'Men' ? (
                        <Link to = {`/`} className = "category" key = {category}>
                            {<span style = {{color: selectedCategory === category ? COLORS.PRIMARY : COLORS.SILVERSAND}}> {category} </span>}
